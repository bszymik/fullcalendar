    ...
    */
        ...
        if (allDay) {
            ...
            if (slots && snapOnSlots) {
                ...
                for (var i = 0; i < slots.length; i++) {
                    ...
                    var duration = moment.duration(slot.start);
                    ...
                    if (startTime.isSame(zonedStart)) {
                        ...
                        var duration_1 = moment.duration(slot.end);
                        end.set({
                            'hour': duration_1.get('hour'),
                            'minute': duration_1.get('minute'),
                            'second': duration_1.get('second')
                        });
                        break;
                    }
                }
    ...
    Calendar.prototype.computeSnapPolicyEnlarge = function (startDate, endDate, slots) {
        ...
        for (var i = 0; i < slots.length; i++) {
            ...
            var endTime = startDate.clone().time(slot.end);
...
var TimeGrid = /** @class */ (function (_super) {
    ...
    TimeGrid.prototype.processOptions = function () {
        ...
        if (slots && Array.isArray(slots)) {
            ...
            if (slots.length >= 1) { // require at least 1 slot to display properly
                ...
                var showSlotEndTime = this.opt('showSlotEndTime');
                if (showSlotEndTime !== false) { // defaults to true
                    this.showSlotEndTime = true;
                }
                var showMinorSlotTime = this.opt('showMinorSlotTime');
    ...
    TimeGrid.prototype.renderSlatRowHtml = function () {
        ...
        if (slots) {
            ...
            var startTime = void 0;
            var endTime = void 0;
            var nextStartTime = void 0;
            var breakHtml = '';
            var breakHeight = void 0;
            var slotHeight = void 0;
            var start = calendar.msToUtcMoment(dateProfile.renderUnzonedRange.startMs);
            for (var i = 0; i < slots.length; i++) {
                ...
                if (showMinorSlotTime === true || !slot.minor) {
                    timeHtml = util_1.htmlEscape(startTime.format(this.labelFormat));
    ...
    TimeGrid.prototype.computeTimeTopWithoutSlots = function (time) {
    ...
    TimeGrid.prototype.computeTimeTopWithSlots = function (time) {
        var slots = this.slots;
        var start = this.view.calendar.msToUtcMoment(this.dateProfile.renderUnzonedRange.startMs);
        var time2 = start.clone().time(time); // Convert duration to time;
        var slatIndex = null;
        var isInSlot = false;
        ...
        if (isInSlot) {
            ...
            var previousEndTime = start.clone().time(slots[slatIndex - 1].end);
    ...
    TimeGrid.prototype.computeSnapTime = function (snapIndex) {
        ...
        if (slots && this.snapOnSlots) {
            var start = this.view.calendar.msToUtcMoment(this.dateProfile.renderUnzonedRange.startMs);
            var beginTime = start.clone();
            var rowTime = start.clone().time(slots[Math.min(snapIndex, slots.length - 1)].start);
